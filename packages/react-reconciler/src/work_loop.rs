...
impl WorkLoop {
    pub fn new() -> Self {
        ...
        Self {
            ...
            complete_work: CompleteWork::new((*host_config).clone()),
        }
    }
    ...
    fn perform_sync_work_on_root(&mut self, root: Rc<RefCell<FiberRootNode>>) {
        ...
        log!(
            "finished_work {:?}",
            finished_work.clone().unwrap().borrow().tag
        );
    ...
    fn prepare_fresh_stack(&mut self, root: Rc<RefCell<FiberRootNode>>) {
        ...
        log!(
            "prepare_fresh_stack - state_node {:?}",
            root.clone()
                .borrow()
                .current
                .clone()
    ...
    fn complete_unit_of_work(&mut self, fiber: Rc<RefCell<FiberNode>>) {
        ...
        loop {
            let next = self.complete_work.complete_work(node.clone().unwrap().clone());
